...
namespace FreeImageAPI
{
	...
	public struct FIRGBA16 : IComparable, IComparable<FIRGBA16>, IEquatable<FIRGBA16>, IRGBAColor
	{
		/// <summary>
		...
		uint R
		{
			get { return (uint)this.red; }
			set { this.red = (ushort)value; }
		}

		uint G
		{
			get { return (uint)this.green; }
			set { this.green = (ushort)value; }
		}

		uint B
		{
			get { return (uint)this.blue; }
			set { this.blue = (ushort)value; }
		}

		uint A
		{
			get { return (uint)this.alpha; }
			set { this.alpha = (ushort)value; }
		}

		double IRGBAColor.R
		{
			get { return this.R / 65535d; }
			set { this.R = (uint)Math.Round(value * 65535); }
		}

		double IRGBAColor.G
		{
			get { return this.G / 65535d; }
			set { this.G = (uint)Math.Round(value * 65535); }
		}

		double IRGBAColor.B
		{
			get { return this.B / 65535d; }
			set { this.B = (uint)Math.Round(value * 65535); }
		}

		double IRGBAColor.A
		{
			get { return this.A / 65535d; }
			set { this.A = (uint)Math.Round(value * 65535); }
		}
