...
namespace FreeImageAPI
{
	...
	/// intensities of red, green and blue value. The red and blue color component
	/// consume 5 bits (range: 0..31) and the green color component consumes 6 bit
	/// (range: 0..63).
	/// </summary>
	/// <remarks>
	...
	public struct FI16RGB565 : IComparable, IComparable<FI16RGB565>, IEquatable<FI16RGB565>, IRGBAColor
	{
		/// <summary>
		...
		uint R
		{
			get { return ((uint)(this.value & FreeImage.FI16_565_RED_MASK) >> FreeImage.FI16_565_RED_SHIFT); }
			set { this.value = (ushort)(((uint)this.value & (~FreeImage.FI16_565_RED_MASK)) | (value << FreeImage.FI16_565_RED_SHIFT)); }
		}

		uint G
		{
			get { return ((uint)(this.value & FreeImage.FI16_555_GREEN_MASK) >> FreeImage.FI16_565_GREEN_SHIFT); }
			set { this.value = (ushort)(((uint)this.value & (~FreeImage.FI16_565_GREEN_MASK)) | (value << FreeImage.FI16_565_GREEN_SHIFT)); }
		}

		uint B
		{
			get { return ((uint)(this.value & FreeImage.FI16_565_BLUE_MASK) >> FreeImage.FI16_565_BLUE_SHIFT); }
			set { this.value = (ushort)(((uint)this.value & (~FreeImage.FI16_565_BLUE_MASK)) | (value << FreeImage.FI16_565_BLUE_SHIFT)); }
		}

		double IRGBAColor.R
		{
			get { return this.R / 31d; }
			set { this.R = (uint)Math.Round(value * 31); }
		}

		double IRGBAColor.G
		{
			get { return this.G / 63d; }
			set { this.G = (uint)Math.Round(value * 63); }
		}

		double IRGBAColor.B
		{
			get { return this.B / 31d; }
			set { this.B = (uint)Math.Round(value * 31); }
		}

		double IRGBAColor.A { get { return 1d; } set { } }
