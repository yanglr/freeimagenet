...
namespace FreeImageAPI
{
	...
	public static partial class FreeImage
	{
		...
		/// </summary>
		/// <param name="fif">The <see cref="FREE_IMAGE_FORMAT"/> of the plugin.</param>
		/// <returns>True if the plugin can load only the header, else false.</returns>
		[DllImport(FreeImageLibrary, EntryPoint = "FreeImage_FIFSupportsNoPixels")]
		public static extern bool FIFSupportsNoPixels(FREE_IMAGE_FORMAT fif);
		...
		/// <summary>
		/// Calculates (or at least estimates) the total memory usage of a FreeImage bitmap.
		/// </summary>
		/// <param name="dib">Handle to a FreeImage bitmap</param>
		/// <returns>Memory usage in bytes.</returns>
		/// <remarks>
		/// <para>
		/// This includes the ICC profile size, the size of the embedded thumbnail (if any), the memory
		/// required for all the metadata, as well as any FreeImage internal (housekeeping) memory.
		/// </para>
		/// <para>
		/// There is another difference with the function <see cref="FreeImage.GetDIBSize"/>.
		/// <see cref="FreeImage.GetMemorySize"/> returns meaningful values for views, that are images that share
		/// the DIB data with other FIBITMAPs (see <see cref="FreeImage.CreateView"/>) or even with other non-
		/// FreeImage memory (see <see cref="FreeImage.ConvertFromRawBitsEx"/>)
		/// </para>
		/// <para>
		/// Additionally, <see cref="FreeImage.GetMemorySize"/> works with images loaded with
		/// the <see cref="FREE_IMAGE_LOAD_FLAGS.FIF_LOAD_NOPIXELS"/> flag as well.
		/// </para>
		/// </remarks>
		/// <seealso cref="FreeImage.GetDIBSize"/>
		[DllImport(FreeImageLibrary, EntryPoint = "FreeImage_GetMemorySize")]
		public static extern uint GetMemorySize(FIBITMAP dib);
		...
		/// </summary>
		/// <param name="dib">The image to be enlarged or shrunken.</param>
		/// <param name="left">The number of pixels, the image should be enlarged on its left side. Negative values shrink the image on its left side.</param>
		/// <param name="top">The number of pixels, the image should be enlarged on its top side. Negative values shrink the image on its top side.</param>
		/// <param name="right">The number of pixels, the image should be enlarged on its right side. Negative values shrink the image on its right side.</param>
		/// <param name="bottom">The number of pixels, the image should be enlarged on its bottom side. Negative values shrink the image on its bottom side.</param>
		...
		/// <returns>A pointer to a newly allocated enlarged or shrunken image on success, NULL otherwise</returns>
		/// <remarks>
		/// <para>
		/// This function fails if either the input image is NULL or the pointer to the color is NULL,
		/// while at least one of left, top, right and bottom is greater than zero. This function also returns NULL,
		/// if the new image's size will be negative in either x or y-direction.
		/// </para>
		/// <para>
		...
		/// the same as calling function <see cref="FreeImage.Clone"/>; setting all parameters left, top, right and
		/// bottom to value equal to or smaller than zero, may easily be substituted by a call to function
		/// <see cref="FreeImage.Copy"/>. Both these cases produce a new image, which is guaranteed not to be
		/// larger than the input image. Thus, since the specified color is not needed in these cases, the
		/// pointer color may be NULL.
		/// </para>
		/// <para>
		/// Both parameters color and options work according to function <see cref="FreeImage.FillBackground"/>. So,
		/// please refer to the documentation of <see cref="FreeImage.FillBackground"/> to learn more about
		...
		/// <summary>
		/// Provides support for rescaling only a rectangular area of an image.
		/// </summary>
		/// <param name="dib"></param>
		/// <param name="dst_width"></param>
		...
		/// <returns>A pointer to an image containing a rescaled version of the selected rectangle on success, otherwise <c>null</c>.</returns>
		/// <remarks>
		/// <para>
		/// <see cref="FreeImage.RescaleRect"/> is basically a much faster version of using <see cref="FreeImage.Copy"/> and <see cref="FreeImage.Rescale"/>.
		/// </para>
		/// <para>
		/// By using flag <see cref="FREE_IMAGE_RESCALE_FLAGS.FI_RESCALE_TRUE_COLOR"/>, one can ensure not getting a palletized 8-bit
		...
		/// gray colors only (r=g=b). By default, <see cref="FreeImage.Rescale"/> returns an image with the smallest
		...
		/// </para>
		/// </remarks>
		[DllImport(FreeImageLibrary, EntryPoint = "FreeImage_RescaleRect")]
		public static extern FIBITMAP RescaleRect(FIBITMAP dib, int dst_width, int dst_height, int left, int top, int right, int bottom, FREE_IMAGE_FILTER filter, FREE_IMAGE_RESCALE_FLAGS flags);
		...
		/// </summary>
		/// <param name="dib">The FreeImage bitmap on which to create the view.</param>
		...
		/// the view much faster than similar solutions using <see cref="Copy"/>, a view uses some
		/// private memory that needs to be freed by calling <see cref="Unload"/> on the view's
		...
		/// </para>
		/// </remarks>
		/// <returns>A handle to the newly created view or returns NULL if the view was not created.</returns>
		[DllImport(FreeImageLibrary, EntryPoint = "FreeImage_CreateView")]
		public static extern FIBITMAP CreateView(FIBITMAP dib, int left, int top, int right, int bottom);
		...
		[DllImport(FreeImageLibrary, EntryPoint = "FreeImage_FillBackground")]
		internal static extern bool FillBackground(FIBITMAP dib, IPixelValue color, FREE_IMAGE_COLOR_OPTIONS options);
